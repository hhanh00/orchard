...
#[cfg(feature = "circuit")]
impl<T, Pr: NoteFlavor> Action<T, Pr> {
...
impl BundleVersion {
    ...
    pub fn default_flags(&self) -> Flags {
        Flags::from_parts(true, true, self.permits_cross_address_transfers(), false)
    }
...
pub struct Flags {
    ...
    /// This is only used by the ZSA circuit path.
    zsa_enabled: bool,
}
...
impl Flags {
    ...
    /// The flag set with outputs disabled and ZSA enabled.
    pub const OUTPUTS_DISABLED_WITH_ZSA: Flags = Flags {
        spends_enabled: true,
        outputs_enabled: false,
        cross_address_enabled: true,
        zsa_enabled: true,
    ...
    /// Serialize flags to a byte using ZSA encoding.
    ///
    /// This is used by the ZSA circuit path, where bit 2 encodes `zsa_enabled`.
    pub fn to_byte_zsa(&self) -> u8 {
        let mut value = 0u8;
    ...
    /// Parses flags from a single byte using ZSA encoding.
    ///
    /// This is used by the ZSA circuit path, where bit 2 encodes `zsa_enabled`.
    pub fn from_byte_zsa(value: u8) -> Option<Self> {
        if value & !(FLAG_SPENDS_ENABLED | FLAG_OUTPUTS_ENABLED | FLAG_ZSA_ENABLED) != 0 {
            return None;
        }
        Some(Self {
...
#[derive(Clone)]
pub struct Bundle<T: Authorization, V, Pr: NoteFlavor = NormalFlavor> {
    /// The list of actions that make up this bundle.
    actions: NonEmpty<Action<T::SpendAuth, Pr>>,
...
impl<T: Authorization, V: fmt::Debug, Pr: NoteFlavor> fmt::Debug for Bundle<T, V, Pr> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        /// Helper struct for debug-printing actions without exposing `NonEmpty`.
        struct Actions<'a, T, Pr: NoteFlavor>(&'a NonEmpty<Action<T, Pr>>);
        impl<T: fmt::Debug, Pr: NoteFlavor> fmt::Debug for Actions<'_, T, Pr> {
            fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl<T: Authorization, V, Pr: NoteFlavor> Bundle<T, V, Pr> {
    ...
    pub fn actions(&self) -> &NonEmpty<Action<T::SpendAuth, Pr>> {
        &self.actions
    }
    ...
    ) -> Vec<(usize, IncomingViewingKey, Note<Pr>, Address, [u8; 512])> {
    ...
    ) -> Vec<(usize, OutgoingViewingKey, Note<Pr>, Address, [u8; 512])> {
        self.actions
            .iter()
            .enumerate()
            .filter_map(|(idx, action)| {
...
impl<T: Authorization, V: Copy + Into<i64>, Pr: NoteFlavor> Bundle<T, V, Pr> {
...
impl<V, Pr: NoteFlavor> Bundle<EffectsOnly, V, Pr> {
...
impl<V: DynamicUsage, Pr: NoteFlavor> DynamicUsage for Bundle<Authorized, V, Pr> {
    ...
    fn dynamic_usage_bounds(&self) -> (usize, Option<usize>) {
        let bounds = (
            ...
            self.burn.dynamic_usage_bounds(),
        );
        (
...
/// Whether to enforce proof size checks.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ProofSizeEnforcement {
    /// Check that the proof is exactly the canonical size.
    Strict,
    /// Do not check proof size (used for pre-NU6.2 bundles).
    Skip,
}
