...
/// Witness data for the nullifier non-membership proof (PIR-compatible).
#[derive(Clone, Debug)]
pub struct VotePowerInfo {
    /// Domain nullifier (election-specific)
    pub dnf: Nullifier,
    /// Lower bound of the gap range containing nf_old
    pub nf_low: pallas::Base,
    /// Width of the gap range (high - low)
    pub nf_width: pallas::Base,
    /// Leaf position in the Poseidon Merkle tree
    pub nf_pos: u32,
    /// Merkle authentication path (29 sibling hashes)
    pub nf_path: [pallas::Base; NF_MERKLE_DEPTH],
}
...
impl VotePowerInfo {
    pub(crate) fn from_parts(
        dnf: Nullifier,
        nf_low: pallas::Base,
        nf_width: pallas::Base,
        nf_pos: u32,
        nf_path: [pallas::Base; NF_MERKLE_DEPTH],
    ) -> Self {
        VotePowerInfo {
            dnf,
            nf_low,
...
impl plonk::Circuit<pallas::Base> for Circuit {
    ...
    fn configure(meta: &mut plonk::ConstraintSystem<pallas::Base>) -> Self::Config {
        ...
        // Constrain nf_in_range (PIR-compatible range check)
    ...
    fn synthesize(
        ...
        let (
            ...
            // Witness nf_low (lower bound of gap range).
            let nf_low = assign_free_advice(
                layouter.namespace(|| "witness nf_low"),
                config.advices[0],
                self.nf_low,
            )?;

            // Witness nf_width (width of gap range).
            let nf_width = assign_free_advice(
        ...
        // Nullifier Merkle path validity check (Poseidon-based, PIR-compatible).
        // Compute leaf_hash = Poseidon(nf_low, nf_width), then walk up 29 levels.
        let nf_root = {
            // Compute leaf commitment: Poseidon(nf_low, nf_width)
            let leaf_hash = {
                ...
                >::init(
                    ...
                    layouter.namespace(|| "NF leaf Poseidon init"),
                )?;
                poseidon_hasher.hash(
                    layouter.namespace(|| "NF leaf hash = Poseidon(low, width)"),
                    [nf_low.clone(), nf_width.clone()],
                )?
            };

            // Walk up the Poseidon Merkle tree for NF_MERKLE_DEPTH levels
            let mut current = leaf_hash;
            let nf_pos_val = self.nf_pos;

            for level in 0..NF_MERKLE_DEPTH {
                // Witness the sibling hash at this level
                let sibling = assign_free_advice(
                    layouter.namespace(|| format!("nf_path sibling level {}", level)),
                    config.advices[0],
                    self.nf_path.map(|p| p[level]),
                )?;

                // Determine order: if bit is 0, current is left; if 1, current is right
                let bit = nf_pos_val.map(|pos| (pos >> level) & 1);

                let (left, right) = {
                    let left = assign_free_advice(
                        layouter.namespace(|| format!("nf merkle left level {}", level)),
                        config.advices[0],
                        bit.zip(current.value().zip(sibling.value())).map(
                            |(b, (cur, sib))| {
                                if b == 0 { *cur } else { *sib }
                            },
                        ),
                    ...
                    let right = assign_free_advice(
                        layouter.namespace(|| format!("nf merkle right level {}", level)),
                        config.advices[0],
                        bit.zip(current.value().zip(sibling.value())).map(
                            |(b, (cur, sib))| {
                                if b == 0 { *sib } else { *cur }
                ...
                let parent: halo2_proofs::circuit::AssignedCell<pallas::Base, pallas::Base> = {
                    let poseidon_hasher = PoseidonHash::<
                        _,
                        _,
                        poseidon::P128Pow5T3,
                    ...
                    >::init(
                        config.poseidon_chip(),
                        layouter.namespace(|| format!("NF Poseidon init level {}", level)),
                    )?;
                    poseidon_hasher.hash(
                        layouter.namespace(|| format!("NF Poseidon hash level {}", level)),
                        [left, right],
                    )?
                };
        ...
        // Range constraint on nf_old: prove nf_old - nf_low <= nf_width
        let nf_in_range = nf_interval.check_in_range(
            layouter.namespace(|| "nf in [nf_low, nf_low + nf_width]"),
            nf_old.inner().clone(),
            nf_low,
            nf_width,
        )?;
